...
Die jetzige View ist auf die Fehlerquelle \brand{KeepRight} (siehe Kapitel \ref{datenquellen-keepright}) angepasst.
...
Falls dies nicht geht oder es sich um neue Arten von Fehlern handelt, müssen zusätzliche Fehlertypen eingeführt werden (siehe Abschnitt \ref{additional-error-type}).
...
Wichtig ist es dabei, den Endbenutzer nicht aus den Augen zu verlieren.
...
Wird als View-Typ \inlinecode{select} gewählt, müssen in der Tabelle \inlinecode{kort.answer} die möglichen Antworten eingetragen werden.
...
Derzeit ist der Login über Googles \gls{OAuth}-Dienst möglich. 
