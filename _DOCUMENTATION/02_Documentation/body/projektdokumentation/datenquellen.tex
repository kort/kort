...
Es galt deshalb zuerst eine geeignete Quelle für Fehlerdaten von \brand{OpenStreetMap} zu finden.
...
\item Die Fehler sollten sich immer nur auf einen \gls{Tag} des betroffenen \brand{OpenStreetMap}-Objekt beziehen. Dadurch vereinfacht sich das Zurückschreiben der Daten zu \brand{OpenStreetMap}.
\end{itemize}
...
\subsection{MapDust}
\brand{MapDust} ist ein Bug-Tracker der Firma skobbler.
Er ist \brand{OpenStreetBugs} (siehe Abschnitt \ref{openstreetbugs}) sehr ähnlich, konzentriert sich aber mehr auf Fehler beim Routing\todo{Routing ins Glossar}.
