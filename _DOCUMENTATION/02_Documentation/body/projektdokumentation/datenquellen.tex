...
Das Shellskript befindet sich unter \inlinecode{/server/database/keepright\_setup.sh}.
...
\hline 
schema & Schemabezeichner welcher die Region gemäss Platet-Karte angibt  \\
\hline
error\_id & Fehler-ID pro Schema  \\
\hline
error\_type & Typisierung des Fehlers  \\
\hline
error\_name & Name des Fehlertyps  \\
\hline
object\_type & OpenStreetMap-Typ des Objekts (\gls{Node}, \gls{Way} oder \gls{Relation})  \\ 
\hline
object\_id & OpenStreetMap-ID des Objekts  \\
\hline
state & Status des Fehlers (new, reopened, ignore\_temporarily, ignore)  \\
\hline
msgid & Fehlermeldung mit Platzhaltern (\$1 - \$5)  \\
\hline
txt1\newline
txt2\newline
txt3\newline
txt4\newline
txt5 & Texte für Platzhalter in msgid  \\
\hline
first\_occurrence & Zeitpunkt, an dem dieser Fehler das erste Mal entdeckt wurde (MESZ)  \\
\hline
last\_checked & Zeitpunkt an dem dieser Fehler das letzte Mal entdeckt wurde (MESZ)  \\
\hline
object\_timestamp & Letzter Änderungzeitpunkt am Objekt  \\
\hline
user\_name & Benutzer, welcher das Objekt zuletzt verändert hat  \\
\hline
lat\newline
lon & Längen- und Breitengrad des Fehlers  \\
\hline
comment & Kommentar zum Fehler  \\
\hline
comment\_timestamp & Zeitpunkt des Kommentars zum Fehler  \\
\hline
\end{tabular}
