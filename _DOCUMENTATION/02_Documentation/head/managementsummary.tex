...
Einige Tools wie \brand{KeepRight}\footnote{\url{http://keepright.ipax.at/}} oder \brand{Osmose}\footnote{\url{http://osmose.openstreetmap.fr/map/}} (Abbildung \ref{image-osmose-screenshot}) berechnen aus den Karten-Rohdaten die vorhanden Fehler.
...
So gibt es Navigationsgeräte-Herstellen welche die Daten aus \brand{OpenStreetMap} zur Routenberechnung verwenden und dabei ihren Benutzern ermöglichen, falsche Routen zu melden.
...
Das Themengebiet der \gls{Gamification} bietet viele Ansätze um eine normalerweise "`uninteressante"' Aufgabe spannend zu gestalten.
...
Es ist wichtig, die Einstiegsschwelle tief zu halten und auch einem unerfahrenen Benutzer Werkzeuge zu geben, die einen gewissen Spass vermitteln.\todo{Abschnitt überarbeiten}
...
\section*{Ausblick}
Die App hat ein grosses Potential Personen, welche sich bislang nicht mit der Thematik \brand{OpenStreetMap} befasst haben, für das Projekt zu begeistern.
...
Die wichtigste noch offene Erweiterung stellt aber das noch fehlenden Zurückschreiben der eingetragenen Lösungen zu \brand{OpenStreetMap} dar.
