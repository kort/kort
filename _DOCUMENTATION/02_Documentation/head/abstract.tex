...
Zur Behebung dieser Fehler ist die cross-platform \gls{WebApp} \textsc{Kort} entwickelt worden.
Diese ist in JavaScript geschrieben und basiert auf dem \emph{Sencha Touch 2}-Framework.
Im Backend kommt eine \emph{PostgreSQL}-Datenbank zum Einsatz. Die komplette Kommunikation ist mit \gls{REST}-Schnittstellen realisiert.

Mit \textsc{Kort} soll das Verbessern von Karten-Daten auf unterhaltsame Weise ermöglicht werden.
Dem Benutzer werden dazu die \gls{OpenStreetMap}-Fehler direkt auf einer Karte angezeigt.
...
Die App ist ein Prototyp der zurzeit auf den deutschsprachigen Raum beschränkt ist.
\end{abstract}