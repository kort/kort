 ...
 *
 * To use this component you must include an additional JavaScript file from Leaflet:
 *
...
Ext.define('Ext.ux.LeafletMap', {
	...
	renderMap: function() {
		...
		if (l) {
            ...
            if (!mapOptions.hasOwnProperty('center') || !(mapOptions.center instanceof l.LatLng)) {
                mapOptions.center = new l.LatLng(47.36865, 8.539183); // default: Zuerich
            }
			
            me.setTileLayer(l.tileLayer(me.getTileLayerUrl(), me.getTileLayerOptions()));
			tileLayer = me.getTileLayer();
            mapOptions.layers = [tileLayer];
            
			me.setMap(l.map(element.dom, mapOptions));
			map = me.getMap();
	...
	setMapCenter: function(coordinates) {
        ...
        if (l) {
			...
			coordinates = coordinates || new l.LatLng(47.36865, 8.539183);
			
            if (coordinates && !(coordinates instanceof l.LatLng) && 'latitude' in coordinates) {
