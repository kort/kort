...
for seq in `psql -qAt -c "select sequence_schema || '.' || sequence_name from information_schema.sequences where sequence_schema = '$DB_SCHEMA';" $DB_NAME` ; do  psql -c "alter $seq owner to $DB_OWNER" $DB_NAME ; done
