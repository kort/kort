...
FROM osm_errors.dblink('dbname=gis_db port=8080 host=152.96.80.44 user=readonly',
     'SELECT ceiling(ST_Y(ST_Centroid(way))::numeric * 100) AS lat, ceiling(ST_X(ST_Centroid(way))::numeric * 100) AS lon, ST_Transform(ST_Centroid(way),4326) AS geom, osm_id, name AS txt1
			FROM osm_poi
