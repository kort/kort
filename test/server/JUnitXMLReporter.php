...
class JUnitXMLReporter extends SimpleReporter {
    ...
    function paintMessage($message) {
        ...
        $this->appendMessage($message , 'system-out');
    }
    function paintFormattedMessage($message) {
        $this->paintMessage($message);
    }
    function terminateAbnormally($message) {
        $this->appendMessage($message , 'failure');
    }
    
    function appendMessage($message , $type='failure') {
        if (!$this->currCase) {
            error_log("!! currCase was not set.");
            return;
        }

        $ch = $this->doc->createElement($type);
        if ($type=='failure'){
